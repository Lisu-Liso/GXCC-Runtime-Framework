...
namespace GXCCRuntimeFramework{
    ...
    bool isGXCCrunning(){
        char str[]="GXCC_Core.exe"; // To solve the Warning.
        return ProcessName2Pid(str);
    }
    bool GetStartWithGXCC(std::string ExtensionID){
        std::string gxccdir=getdirpath();
        if(iserrorinrunning(gxccdir)) return 0;
        std::string startfile=gxccdir+"Start\\StartExtensionList.GXl";
        FILE *stream=fopen(startfile.c_str(),"r");
        if(!stream) return 0;
        int c=0;
    ...
    bool SetStartWithGXCC(std::string ExtensionID,bool Start=1){
        if(Start){
            if(GetStartWithGXCC(ExtensionID)) return 1;
            std::string gxccdir;
            gxccdir=getdirpath();
            if(iserrorinrunning(gxccdir)) return 1;
            std::string startfilepath=gxccdir+"Start\\StartExtensionList.GXl";
            FILE *stream;
            stream=fopen(startfilepath.c_str(),"a+");
            if(!stream) return 0;
            fputs(ExtensionID.c_str(),stream);
            fclose(stream);
        }else{
            ...
            fclose(stream);
            stream=fopen(startfile.c_str(),"w");
            fputs(filecontent.c_str(),stream);
            fclose(stream);
        }
        return 1;
    }
